...
export default function Header({
  ...
  return (
    <header className="fixed flex flex-col w-screen justify-center py-[1rem] sm:py-[2.313rem] z-50 bg-gradient-to-b from-primary-100 to-white">
      <div className="global-responsive-margin">
          ...
          <div className="flex z-40 items-center justify-between">
            ...
            {!isAI && (
              <>
                ...
                <div
                  className="sm:hidden relative block cursor-pointer"
              ...
              <BreadcrumbList>
                ...
                {pathNameArray.map((item: string, index: number) => {
                  if (index != pathNameArray.length - 1) {
                    ...
                    return (
                      <BreadcrumbItem key={index}>
                        <BreadcrumbPage className=" font-body-font bg-secondary-200 py-1 px-2 rounded-full">
                          {capitalizeFirstLetter(recipeName || item)}
                        </BreadcrumbPage>
                      </BreadcrumbItem>
                    );
                  }
